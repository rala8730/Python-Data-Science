...
def chisquare_pvalue(obs, ex):
    ...
    """
    #chi=(obs[0][0]-ex[0][0])**2 / ex[0][0] + (obs[0][1]-ex[0][1])**2 / ex[0][1] + (obs[1][0]-ex[1][0])**2 / ex[1][0] + (obs[1][1]-ex[1][1])**2 / ex[1][1]
    (_,a)=chisquare([obs[0][0],obs[0][1],obs[1][0],obs[1][1]],[ex[0][0],ex[0][1],ex[1][0],ex[1][1]],2)
...
class BigramFinder:
    ...
    def add_sentence(self, sentence):
        ...
        for ll, rr in bigrams(sentence):
            ...
            # Your code here
            if (ll,rr) in self.bigramCounter.keys():
                self.bigramCounter[(ll,rr)]=self.bigramCounter[(ll,rr)]+1
            else:
                self.bigramCounter[(ll,rr)]=1
    ...
    def valid_bigrams(self):
        ...
        # Your code here
        result=[]
        for key in self.bigramCounter.keys():
            if self.bigramCounter[key] > self._min_ngram:
                result.append(key)
