...
class BigramLanguageModel:
    ...
    def train_seen(self, word):
        ...
        # Add your code here!  
        self._vocab.add(word) #adding the new word in vocab
    ...
    def generate(self, context):
        ...
        # Your code here
        nextword = '' #next word is blank
        maximum = kNEG_INF # when maximim is given 
        for v_word in self._vocab: #words in vocab
            lm_probability=self.laplace(context,v_word) # prob is calling laplace function 
            #and taking its argument 
            if(lm_probability > maximum):
                #so when the word from lm_probability is  greater than max
                nextword = v_word
                # replace the given and assign it equal to new word
                maximum = lm_probability
                #resetting lm probability
        return nextword
    ...
    def laplace(self, context, word):
        ...
        # Add your code here
        #occur=self.thecounter[context,word]+1
        #initial=0
        #for ww in self.thecounter.items():
         #   if ww[0][0]==context:
          #      initial+=ww[1]
        #t_vocab=len(self._vocab)
        #final=ww+t_vocab
        #prob=initial/t_vocab
        #val=log(prob)
        #val = 0.0
        counter=0
        
        for i in context:#times context appeared
            counter+=1
            for j in word:# times word appeared with context
                counter +=1

        lenofvoc=len(self._vocab*1)

        val = counter+lenofvoc
        return val
    ...
    def add_train(self, sentence):
        ...
        for context, word in bigrams(list(self.tokenize_and_censor(sentence))):
            ...
            assert word in self._vocab, "%s not in vocab" % word
            key=(context,word)
            if self.thecounter.keys(key):
                self.thecounter.keys[key]=self.thecounter.keys[key+1]
            else:
                thecounter.keys(key)
